/*
 * Original shader from: https://www.shadertoy.com/view/WlcfRS
 */
...
void init_effects(void) {
  effects[0] = effect(0.125, 0.0, 0.0, 0.0);
  effects[1] = effect(0.125, 0.0, 0.0, 1.0);
  effects[2] = effect(0.125, 0.0, 1.0, 1.0);
  effects[3] = effect(0.125, 1.0, 1.0, 1.0);
  effects[4] = effect(0.125, 1.0, 1.0, 0.0);
  effects[5] = effect(0.125, 1.0, 0.0, 0.0);
  current_effect = effects[5];
}
...
vec4 plane(vec3 ro, vec3 rd, vec3 pp, float pd, vec3 off, float aa, float n) {
  int pi = int(mod(n/PLANE_PERIOD, float(6)));
  if (pi == 0)
