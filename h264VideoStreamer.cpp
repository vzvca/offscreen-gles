...
// Original source testH264VideoStreamer.cpp
// from debian buster package liblivemedia 2018.11.26-1.1
...
void usage (int argc, char **argv, int optind)
{
  ...
  const char *fmt =
    ...
    "    -i /path/to/file          Path to input raw h264 vide file. Defaults to '%s'.\n"
...
void parse (int argc, char **argv)
{
  ...
  while ((opt = getopt (argc, argv, "?hi:s:")) != -1 ) {
    switch( opt ) {
    case '?':  case 'h': usage (argc, argv, 0);
    case 'i':  inputFileName = optarg; break;
    case 's':  streamName = optarg; break;
    default:
